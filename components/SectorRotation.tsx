...
interface Res { data: SectorRow[]; benchmark: SectorRow | null; updatedAt: string; error?: string }
...
export default function SectorRotation() {
  ...
  useEffect(() => {
    fetch('/api/sector')
      .then(r => r.json())
      .then(setRes)
      .catch(() => setRes({ data: [], benchmark: null, updatedAt: '', error: 'fetch failed' }))
                ...
                <tr key={row.ticker} className="border-b border-t-border/40 hover:bg-t-surface/60 transition-colors">
                  ...
                  <td className="px-3 py-2">
                    <div className="h-1 bg-t-border rounded overflow-hidden">
                      <div className="h-full rounded" style={{ width: `${barPct}%`, backgroundColor: row.ret1M >= 0 ? '#00d084' : '#ff4d4d' }} />
                    </div>
                  </td>
